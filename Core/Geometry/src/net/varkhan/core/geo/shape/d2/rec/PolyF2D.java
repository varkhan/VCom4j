 ...
 * @time 12:16 PM
 */
public class PolyF2D extends AbstractShape2D implements Iterable<Point2D> {

    protected int num;
    protected float[] xpts;
    protected float[] ypts;

    protected float x1sm;
    protected float y1sm;
    protected float x2sm;
    protected float y2sm;
    protected float xmin;
    protected float xmax;
    protected float ymin;
    protected float ymax;

    public PolyF2D(float[] xpts, float[] ypts) {
    ...
    public <P extends Point2D> PolyF2D(P... pts) {
        this.num=pts.length;
        this.xpts=new float[num];
        this.ypts=new float[num];
        for(int i=0; i<num; i++) {
            this.xpts[i] = (float)pts[i].xctr();
            this.ypts[i] = (float)pts[i].yctr();
        }
        computeInvariants();
    }
    ...
    public boolean equals(Object o) {
        ...
        if(!(o instanceof PolyF2D)) return false;

        PolyF2D that=(PolyF2D) o;
    ...
    public static int winding(double x, double y, int num, float[] xpts, float[] ypts) {
        if(num<2) return 0;
    ...
    public static double distmin2(double x, double y, int num, float[] xpts, float[] ypts) {
...
//                System.err.println("("+xi+","+yi+") <"+d0+","+di+"> "+p0+" "+dd);
    ...
    public static double distmax2(double x, double y, int num, float[] xpts, float[] ypts) {
        double d = 0;
        for(int i=0; i<num; i++) {
            double xi = xpts[i]-x;
            double yi = ypts[i]-y;

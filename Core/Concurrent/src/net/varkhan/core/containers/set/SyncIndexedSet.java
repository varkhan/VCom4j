package net.varkhan.core.containers.set;
...
public class SyncIndexedSet<Key> implements ConcurrentIndexedSet<Key>, Serializable {
     ...
     */
    public <Par> long visit(final Visitable.Visitor<Key,Par> vis, Par par) {
        rlock.lock();
        try {
            return set.visit(new Visitable.Visitor<Key,Par>() {
                private final long exid=opid;
     ...
     * @return the sum of all positive return values from the visitor
     */
    public <Par> long visit(final IndexedVisitable.IndexedVisitor<Key,Par> vis, Par par) {
        rlock.lock();
        try {
            return set.visit(new IndexedVisitable.IndexedVisitor<Key,Par>() {
                private final long exid=opid;
            ...
            public Iterator<IndexedSet.Entry<? extends Key>> iterator() {
                ...
                try {
                    return new Iterator<IndexedSet.Entry<? extends Key>>() {
                        ...
                        final Iterator<IndexedSet.Entry<? extends Key>> iterator=(Iterator<IndexedSet.Entry<? extends Key>>) set.entries();
                        private long exid=opid;
        ...
        return new Iterable<IndexedSet.Entry<? extends Key>>() {
            public Iterator<IndexedSet.Entry<? extends Key>> iterator() {
                ...
                try {
                    return new Iterator<IndexedSet.Entry<? extends Key>>() {
                        @SuppressWarnings("unchecked")
                        final Iterator<IndexedSet.Entry<? extends Key>> iterator=(Iterator<IndexedSet.Entry<? extends Key>>) set.entries(indexes);
                        private long exid=opid;

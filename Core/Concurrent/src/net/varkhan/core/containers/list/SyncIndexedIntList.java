...
import net.varkhan.base.containers.list.IndexedIntList;
 ...
 * @time 7:18:56 PM
 */
public class SyncIndexedIntList implements ConcurrentIndexedIntList, Serializable {

    private static final long serialVersionUID=1L;

    private final Lock           rlock;
    private final Lock           wlock;
    private final IndexedIntList list;
     ...
     */
    public SyncIndexedIntList(IndexedIntList list) {
        this.list=list;
     ...
     */
    public SyncIndexedIntList(IndexedIntList list, Lock rlock, Lock wlock) {
        this.list=list;
        this.rlock=rlock;
        this.wlock=wlock;
    }
     ...
     */
    public long setIfAbsent(long index, Integer val) {
        wlock.lock();
        try {
     ...
     */
    public long rep(long index, Integer oldval, Integer newval) {
        wlock.lock();
        try {
     ...
     */
    public boolean del(long index, Integer val) {
        wlock.lock();
        try {
     ...
     */
    public <Par> long visit(final Visitor<Integer,Par> vis, Par par) {
        rlock.lock();
        try {
            return list.visit(new Visitor<Integer,Par>() {
                private final long exid=opid;

                public long invoke(Integer obj, Par par) {
                    if(exid!=opid) throw new ConcurrentModificationException();
     ...
     * @return the sum of all positive return values from the visitor
     */
    public <Par> long visit(final IntVisitor<Par> vis, Par par) {
        rlock.lock();
        try {
            return list.visit(new IntVisitor<Par>() {
                private final long exid=opid;

                public long invoke(int obj, Par par) {
                    if(exid!=opid) throw new ConcurrentModificationException();
                    return vis.invoke(obj, par);
                }
            }, par);
     ...
     * @return the sum of all positive return values from the visitor
     */
    public <Par> long visit(final IndexedVisitor<Integer,Par> vis, Par par) {
        rlock.lock();
        try {
            return list.visit(new IndexedVisitor<Integer,Par>() {
                private final long exid=opid;

                public long invoke(long idx, Integer obj, Par par) {
                    if(exid!=opid) throw new ConcurrentModificationException();
    ...
    /**
     * Iterate over each element of the list, and pass it as argument to a
     * visitor's {@link Visitor#visit} method, until this method returns
     * a negative count.
     *
     * @param vis the visitor
     * @param par the control parameter
     ...
     * @return the sum of all positive return values from the visitor
     */
    public <Par> long visit(final IndexedIntVisitor<Par> vis, Par par) {
        rlock.lock();
        try {
            return list.visit(new IndexedIntVisitor<Par>() {
                private final long exid=opid;

                public long invoke(long idx, int obj, Par par) {
                    if(exid!=opid) throw new ConcurrentModificationException();
                    return vis.invoke(idx, obj, par);
                }
            }, par);
    ...
    public IntIterable iterate(final long[] indexes) {
        return new IntIterable() {
            public IntIterator iterator() {
                ...
                try {
                    return new IntIterator() {
                        final IntIterator iterator=list.iterate(indexes).iterator();
                        private long exid=opid;

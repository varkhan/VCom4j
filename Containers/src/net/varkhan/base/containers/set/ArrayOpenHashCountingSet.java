...
public class ArrayOpenHashCountingSet<Key> implements CountingSet<Key>, Serializable {
     ...
     */
    public ArrayOpenHashCountingSet(long size, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public ArrayOpenHashCountingSet(long size) {
     ...
     */
    @SuppressWarnings("unchecked")
    public ArrayOpenHashCountingSet() {
    ...
    public Iterator<? extends Key> iterator() {
        return new Iterator<Key>() {
            ...
            {
                Object k;
                if(c!=0) while(pos<ArrayOpenHashCountingSet.this.capa&&((k=keys[pos])==NULL||k==DEL)) pos++;
            }

            public boolean hasNext() {
                return c!=0&&pos<ArrayOpenHashCountingSet.this.capa;
            }
            ...
            public Key next() {
                ...
                Object k;
                if(--c!=0) do pos++; while(pos<ArrayOpenHashCountingSet.this.capa&&((k=keys[pos])==NULL||k==DEL));
                return key;
            }

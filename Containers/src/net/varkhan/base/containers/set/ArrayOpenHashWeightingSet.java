 ...
 * @time 9:43:13 PM
 */
public class ArrayOpenHashWeightingSet<Key> implements WeightingSet<Key>, Serializable, Cloneable {
     ...
     */
    public ArrayOpenHashWeightingSet(long size, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public ArrayOpenHashWeightingSet(long size) {
     ...
     */
    @SuppressWarnings("unchecked")
    public ArrayOpenHashWeightingSet() {
    ...
    public Iterator<? extends Key> iterator() {
        return new Iterator<Key>() {
            ...
            {
                Object k;
                if(c!=0) while(pos<ArrayOpenHashWeightingSet.this.capa&&((k=keys[pos])==NULL||k==DEL)) pos++;
            }

            public boolean hasNext() {
                return c!=0&&pos<ArrayOpenHashWeightingSet.this.capa;
            }
            ...
            public Key next() {
                ...
                Object k;
                if(--c!=0) do pos++; while(pos<ArrayOpenHashWeightingSet.this.capa&&((k=keys[pos])==NULL||k==DEL));
                return key;
            }
     ...
     */
    @SuppressWarnings("unchecked")
    public ArrayOpenHashWeightingSet<Key> clone() {
        ArrayOpenHashWeightingSet<Key> c;
        try {
            c=(ArrayOpenHashWeightingSet<Key>) super.clone();
        }

...
public class ArrayOpenHashSet<Key> implements Set<Key>, Serializable {
     ...
     */
    public ArrayOpenHashSet(long size, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public Iterator<Key> iterator() {
        return new Iterator<Key>() {
            ...
            {
                Object k;
                if(c!=0) while(pos<ArrayOpenHashSet.this.capa&&((k=keys[pos])==NULL||k==DEL)) pos++;
            }

            public boolean hasNext() {
                return c!=0&&pos<ArrayOpenHashSet.this.capa;
            }

            @SuppressWarnings("unchecked")
            public Key next() {
                if(!hasNext()) throw new NoSuchElementException();
                Key key=(Key) keys[(last=pos)];
                Object k;
                if(--c!=0) do pos++; while(pos<ArrayOpenHashSet.this.capa&&((k=keys[pos])==NULL||k==DEL));
                return key;
            }

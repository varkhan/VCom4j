...
public class ArrayOpenHashIndexedObj2DoubleMap<Key> implements IndexedObj2DoubleMap<Key>, Serializable {
     ...
     */
    public ArrayOpenHashIndexedObj2DoubleMap(long size, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public ArrayOpenHashIndexedObj2DoubleMap(long size) {
     ...
     */
    @SuppressWarnings("unchecked")
    public ArrayOpenHashIndexedObj2DoubleMap() {
    ...
    public double getDefaultValue() { return defVal; }

    public void setDefaultValue(double def) { defVal=def; }
    ...
    @SuppressWarnings("unchecked")
    public IndexedObj2DoubleMap.Entry<Key> get(long index) {
        if(index<0||index>=this.head) return null;
        if((this.flgs[(int) (index>>3)]&(1<<(index&7)))==0) return null;
        return new Entry((int) index, (Key) this.keys[(int) index]);
    }
    ...
    public void setValue(long index, Double value) {
        ...
        else this.vals[(int) index]=value.doubleValue();
    }
    ...
    public long add(IndexedMap.Entry<Key,Double> item) {
        if(item==null) return -1;
        return add(item.getKey(), item.getValue());
    }

    public long add(IndexedObj2DoubleMap.Entry<Key> item) {
        if(item==null) return -1;
        return add(item.getKey(), item.getDoubleValue());
    }

    public long add(final Key key, final Double val) {
        if(val==null) return add(key, defVal);
        return add(key, val.doubleValue());
    }
...
//            } while( sidx<0 || (sidx>0 && ! (strategy.hash((Key)(kval=getKey(sidx-1)))==hash && strategy.equal((Key)key, (Key)kval) )) );
    ...
    protected void rehash(final int capa) {
        ...
        if(this.keys.length<capa) {
            ...
            this.keys=keys;
            final double[] vals=new double[capa];
        ...
        // Decrease the size of the keys if possible
            ...
            this.keys=keys;
            final double[] vals=new double[knum];
     ...
     */
    private class Entry implements IndexedObj2DoubleMap.Entry<Key> {
        private final int idx;
        private final Key key;

        public Entry(int idx, Key key) {
            this.idx=idx;
            this.key=key;
        }
        ...
        public Double getValue() { return vals[idx]; }

        public double getDoubleValue() { return vals[idx]; }

        public Double setValue(Double val) {
            double old=vals[idx];
            if(val==null) vals[idx]=defVal;
            else vals[idx]=val.doubleValue();
            return old;
        }
    ...
    public Index indexes() {
        return new Index() {
            ...
            int c=ArrayOpenHashIndexedObj2DoubleMap.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<ArrayOpenHashIndexedObj2DoubleMap.this.capa&&idxs[next]<=0) next++;
                }
                return next<ArrayOpenHashIndexedObj2DoubleMap.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==ArrayOpenHashIndexedObj2DoubleMap.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<ArrayOpenHashIndexedObj2DoubleMap.this.capa&&idxs[pos]<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<ArrayOpenHashIndexedObj2DoubleMap.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        int idx=idxs[(last=pos)]-1;
                        if(--c!=0) do pos++; while(pos<ArrayOpenHashIndexedObj2DoubleMap.this.capa&&idxs[pos]<=0);
                        return (long) idx;
                    }
    ...
    public Iterator<IndexedObj2DoubleMap.Entry<Key>> iterator() {
        return new Iterator<IndexedObj2DoubleMap.Entry<Key>>() {
            /** The position of the next entry to be returned. */
            int pos=0;
            /** The position of the last entry that has been returned. */
            int last=-1;
            ...
            @SuppressWarnings("unchecked")
            public IndexedObj2DoubleMap.Entry<Key> next() {
                if(!hasNext()) throw new NoSuchElementException();
                final int idx=idxs[(last=pos)]-1;
                final Key key=(Key) keys[idx];
                if(--c!=0) do pos++; while(pos<ArrayOpenHashIndexedObj2DoubleMap.this.capa&&idxs[pos]<=0);
    ...
    public <Par> long visit(Visitor<IndexedMap.Entry<Key,Double>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            int idx=idxs[pos];
    ...
    public <Par> long visit(IndexedVisitor<IndexedMap.Entry<Key,Double>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            int idx=idxs[pos];
    ...
    public <Par> long visit(IndexedMapVisitor<Key,Double,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            int idx=idxs[pos];
    ...
    public Iterable<IndexedObj2DoubleMap.Entry<Key>> iterate(final long[] indexes) {
        return new Iterable<IndexedObj2DoubleMap.Entry<Key>>() {
            public Iterator<IndexedObj2DoubleMap.Entry<Key>> iterator() {
                return new Iterator<IndexedObj2DoubleMap.Entry<Key>>() {
                    ...
                    public IndexedObj2DoubleMap.Entry<Key> next() {
                        ...
                        return new Entry(idx, (Key) keys[idx]);
                    }
    ...
    public Iterable<IndexedObj2DoubleMap.Entry<Key>> iterate(final java.lang.Iterable<Long> indexes) {
        return new Iterable<IndexedObj2DoubleMap.Entry<Key>>() {
            public Iterator<IndexedObj2DoubleMap.Entry<Key>> iterator() {
                return new Iterator<IndexedObj2DoubleMap.Entry<Key>>() {
    ...
    public Iterable<IndexedObj2DoubleMap.Entry<Key>> iterate(final Indexable indexes) {
        return new Iterable<IndexedObj2DoubleMap.Entry<Key>>() {
            public Iterator<IndexedObj2DoubleMap.Entry<Key>> iterator() {
                return new Iterator<IndexedObj2DoubleMap.Entry<Key>>() {
                    /** An iterator over indexes */
    ...
    public IndexedContainer<Key> keys() {
        return new IndexedContainer<Key>() {
            public long size() { return ArrayOpenHashIndexedObj2DoubleMap.this.size(); }
            public boolean isEmpty() { return ArrayOpenHashIndexedObj2DoubleMap.this.isEmpty(); }
            public long head() { return ArrayOpenHashIndexedObj2DoubleMap.this.head(); }
            public boolean has(long index) { return ArrayOpenHashIndexedObj2DoubleMap.this.has(index); }
            public Key get(long index) { return ArrayOpenHashIndexedObj2DoubleMap.this.getKey(index); }
            public Index indexes() { return ArrayOpenHashIndexedObj2DoubleMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return ArrayOpenHashIndexedObj2DoubleMap.this.iterateIndexes(); }
...
//                        vals[last]=0;
            ...
            public <Par> long visit(Visitor<Key,Par> vis, Par par) {
                ...
                while(pos<capa) {
                    ...
                    @SuppressWarnings("unchecked")
                    long r=vis.invoke((Key) keys[idx-1], par);
                    if(r<0) return c;
                    c+=r;
                    pos++;
                }
...
//                                vals[last]=0;
    ...
    public IndexedDoubleContainer values() {
        return new IndexedDoubleContainer() {
            ...
            public boolean isEmpty() { return ArrayOpenHashIndexedObj2DoubleMap.this.isEmpty(); }
            public long head() { return ArrayOpenHashIndexedObj2DoubleMap.this.head(); }
            public double getDefaultValue() {  return ArrayOpenHashIndexedObj2DoubleMap.this.getDefaultValue(); }
            public boolean has(long index) { return ArrayOpenHashIndexedObj2DoubleMap.this.has(index); }
            public Double get(long index) { return ArrayOpenHashIndexedObj2DoubleMap.this.getValue(index); }
            public double getDouble(long index) { return ArrayOpenHashIndexedObj2DoubleMap.this.getDoubleValue(index); }
            public Index indexes() { return ArrayOpenHashIndexedObj2DoubleMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return ArrayOpenHashIndexedObj2DoubleMap.this.iterateIndexes(); }

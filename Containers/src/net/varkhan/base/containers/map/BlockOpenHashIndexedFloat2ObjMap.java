...
import net.varkhan.base.containers.type.FloatHashingStrategy;
 ...
 * @time 9:43:13 PM
 */
public class BlockOpenHashIndexedFloat2ObjMap<Value> implements IndexedFloat2ObjMap<Value>, Serializable, Cloneable {
     ...
     */
    protected float defKey=0;

    /**
     * The hash strategy of this map
     */
    protected FloatHashingStrategy strategy;
     ...
     */
    public BlockOpenHashIndexedFloat2ObjMap(long size, int blockshift, float loadfact, float growfact, final FloatHashingStrategy strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
        ...
        this.keys=new float[bnum][];
        this.vals=new Object[bnum][];
    }
     ...
     * @param size the expected number of elements in the map
     */
    public BlockOpenHashIndexedFloat2ObjMap(long size) {
        this(size, 10, .75f, 1.5f, FloatHashingStrategy.DefaultHashingStrategy);
    }

    /**
     * Creates a new hash map.
     */
    public BlockOpenHashIndexedFloat2ObjMap() {
        this(11, 10, .75f, 1.5f, FloatHashingStrategy.DefaultHashingStrategy);
    }
    ...
    public float getDefaultKey() { return defKey; }

    public void setDefaultKey(float def) { defKey=def; }
    ...
    private float _getKey(long index) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        float[] kblock=this.keys[kblockpos];
        // No block ??
        return (kblock==null) ? defKey : kblock[kblockoff];
    }

    private Object _getVal(long index) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        Object[] vblock=this.vals[kblockpos];
        // No block ??
        return (vblock==null) ? null : vblock[kblockoff];
    }

    private void _setVal(long index, Object v) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        Object[] vblock=this.vals[kblockpos];
        if(vblock==null) this.vals[kblockpos]=vblock=new Object[this.blocksize];
        vblock[kblockoff]=v;
    }

    private void _setEntry(long index, float k, Object v) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        float[] kblock=this.keys[kblockpos];
        Object[] vblock=this.vals[kblockpos];
        byte[] fblock=this.flgs[kblockpos];
        if(kblock==null) this.keys[kblockpos]=kblock=new float[this.blocksize];
        if(vblock==null) this.vals[kblockpos]=vblock=new Object[this.blocksize];
    ...
    private void _delEntry(long index) {
        ...
        Object[] vblock=this.vals[kblockpos];
        byte[] fblock=this.flgs[kblockpos];
        if(vblock!=null) vblock[kblockoff]=null;
    ...
    public long index(final Float key) {
        if(key==null) return -1;
        return index(key.floatValue());
    }
    ...
    public long add(IndexedMap.Entry<Float,Value> item) {
    ...
    public long add(IndexedFloat2ObjMap.Entry<Value> item) {
        if(item==null) return -1;
        return add(item.getFloatKey(), item.getValue());
    }

    public long add(final Float key, final Value val) {
        if(key==null) return -1;
        return add(key.floatValue(), val);
    }
...
//            } while( sidx<0 || (sidx>0 && ! (hash == this.strategy.hash((Key)(kval=getKey(sidx-1))) && this.strategy.equal(key, (Key)kval))) );
    ...
    protected void rehash(final long capa) {
        ...
        if(head<capa&&this.capa<capa) {
            ...
            if(bnum>this.keys.length) {
                final float[][] keys=new float[bnum][];
                System.arraycopy(this.keys, 0, keys, 0, this.keys.length);
                this.keys=keys;
                final Object[][] vals=new Object[bnum][];
     ...
     */
    @SuppressWarnings("unchecked")
    private class Entry implements IndexedFloat2ObjMap.Entry<Value> {
        private final long  idx;
        private final float key;

        public Entry(long idx, float key) {
            this.idx=idx;
            this.key=key;
        }
        ...
        public float getFloatKey() { return key; }

        public Value getValue() { return (Value) _getVal(idx); }

        public Value setValue(Value val) {
            Value old=(Value) _getVal(idx);
            _setVal(idx, val);
            return old;
        }
    }
    ...
    public Index indexes() {
        return new Index() {
            ...
            long c=BlockOpenHashIndexedFloat2ObjMap.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<BlockOpenHashIndexedFloat2ObjMap.this.capa&&_getIndex(next)<=0) next++;
                }
                return next<BlockOpenHashIndexedFloat2ObjMap.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==BlockOpenHashIndexedFloat2ObjMap.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<BlockOpenHashIndexedFloat2ObjMap.this.capa&&_getIndex(pos)<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<BlockOpenHashIndexedFloat2ObjMap.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedFloat2ObjMap.this.capa&&_getIndex(pos)<=0);
                        return idx;
                    }
    ...
    public Iterator<IndexedFloat2ObjMap.Entry<Value>> iterator() {
        return new Iterator<IndexedFloat2ObjMap.Entry<Value>>() {
            /** The position of the next entry to be returned. */
            long pos=0;
            /** The position of the last entry that has been returned. */
            long last=-1;
            ...
            public IndexedFloat2ObjMap.Entry<Value> next() {
                if(!hasNext()) throw new NoSuchElementException();
                long idx=_getIndex(last=pos)-1;
                if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedFloat2ObjMap.this.capa&&_getIndex(pos)<=0);
    ...
    public <Par> long visit(Visitor<IndexedMap.Entry<Float,Value>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
    ...
    public <Par> long visit(IndexedVisitor<IndexedMap.Entry<Float,Value>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            long idx=_getIndex(pos);
    ...
    public Iterable<IndexedFloat2ObjMap.Entry<Value>> iterate(final java.lang.Iterable<Long> indexes) {
        return new Iterable<IndexedFloat2ObjMap.Entry<Value>>() {
            public Iterator<IndexedFloat2ObjMap.Entry<Value>> iterator() {
                return new Iterator<IndexedFloat2ObjMap.Entry<Value>>() {
    ...
    public Iterable<IndexedFloat2ObjMap.Entry<Value>> iterate(final Indexable indexes) {
        return new Iterable<IndexedFloat2ObjMap.Entry<Value>>() {
            public Iterator<IndexedFloat2ObjMap.Entry<Value>> iterator() {
                return new Iterator<IndexedFloat2ObjMap.Entry<Value>>() {
                    /** An iterator over indexes */
    ...
    public IndexedFloatContainer keys() {
        return new IndexedFloatContainer() {
            public long size() { return BlockOpenHashIndexedFloat2ObjMap.this.size(); }
            public boolean isEmpty() { return BlockOpenHashIndexedFloat2ObjMap.this.isEmpty(); }
            public long head() { return BlockOpenHashIndexedFloat2ObjMap.this.head(); }
            public float getDefaultValue() { return BlockOpenHashIndexedFloat2ObjMap.this.getDefaultKey(); }
            public boolean has(long index) { return BlockOpenHashIndexedFloat2ObjMap.this.has(index); }
            public Float get(long index) { return BlockOpenHashIndexedFloat2ObjMap.this.getKey(index); }
            public float getFloat(long index) { return BlockOpenHashIndexedFloat2ObjMap.this.getFloatKey(index); }
            public Index indexes() { return BlockOpenHashIndexedFloat2ObjMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedFloat2ObjMap.this.iterateIndexes(); }
    ...
    public IndexedContainer<Value> values() {
        return new IndexedContainer<Value>() {
            ...
            public long head() { return BlockOpenHashIndexedFloat2ObjMap.this.head(); }
            public boolean has(long index) { return BlockOpenHashIndexedFloat2ObjMap.this.has(index); }
            public Value get(long index) { return BlockOpenHashIndexedFloat2ObjMap.this.getValue(index); }
            public Index indexes() { return BlockOpenHashIndexedFloat2ObjMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedFloat2ObjMap.this.iterateIndexes(); }
            ...
            public <Par> long visit(Visitor<Value,Par> vis, Par par) {
                ...
                while(pos<capa) {
                    ...
                    idx--;
                    @SuppressWarnings("unchecked")
                    long r=vis.invoke((Value) _getVal(idx), par);
                    if(r<0) return c;
                    c+=r;
                    pos++;
                }
            ...
            public <Par> long visit(IndexedVisitor<Value,Par> vis, Par par) {
                ...
                while(pos<capa) {
                    ...
                    idx--;
                    @SuppressWarnings("unchecked")
                    long r=vis.invoke(idx, (Value) _getVal(idx), par);
                    if(r<0) return c;
                    c+=r;
                    pos++;
                }
     ...
     */
    @SuppressWarnings("unchecked")
    public BlockOpenHashIndexedFloat2ObjMap<Value> clone() {
        BlockOpenHashIndexedFloat2ObjMap<Value> c;
        try {
            c=(BlockOpenHashIndexedFloat2ObjMap<Value>) super.clone();
        }
    ...
    public boolean equals(Object o) {
        if(o instanceof IndexedFloat2ObjMap) {
            IndexedFloat2ObjMap that=(IndexedFloat2ObjMap) o;

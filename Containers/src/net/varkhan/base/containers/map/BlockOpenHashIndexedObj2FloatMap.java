 ...
 * @time 9:43:13 PM
 */
public class BlockOpenHashIndexedObj2FloatMap<Key> implements IndexedObj2FloatMap<Key>, Serializable, Cloneable {
     ...
     */
    public BlockOpenHashIndexedObj2FloatMap(long size, int blockshift, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    private void _setEntry(long index, Object k, float v) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        Object[] kblock=this.keys[kblockpos];
        float[] vblock=this.vals[kblockpos];
...
//            } while( sidx<0 || (sidx>0 && ! (hash == this.strategy.hash((Key)(kval=getKey(sidx-1))) && this.strategy.equal(key, (Key)kval))) );
    ...
    private class Entry implements IndexedObj2FloatMap.Entry<Key> {
        ...
        public Float setValue(Float val) {
            ...
            else _setVal(idx, val.floatValue());
            return old;
        }
    ...
    public Index indexes() {
        return new Index() {
            ...
            long c=BlockOpenHashIndexedObj2FloatMap.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<BlockOpenHashIndexedObj2FloatMap.this.capa&&_getIndex(next)<=0) next++;
                }
                return next<BlockOpenHashIndexedObj2FloatMap.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==BlockOpenHashIndexedObj2FloatMap.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<BlockOpenHashIndexedObj2FloatMap.this.capa&&_getIndex(pos)<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<BlockOpenHashIndexedObj2FloatMap.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedObj2FloatMap.this.capa&&_getIndex(pos)<=0);
                        return idx;
                    }
    ...
    public IndexedContainer<Key> keys() {
        return new IndexedContainer<Key>() {
            public long size() { return BlockOpenHashIndexedObj2FloatMap.this.size(); }
            public boolean isEmpty() { return BlockOpenHashIndexedObj2FloatMap.this.isEmpty(); }
            public long head() { return BlockOpenHashIndexedObj2FloatMap.this.head(); }
            public boolean has(long index) { return BlockOpenHashIndexedObj2FloatMap.this.has(index); }
            public Key get(long index) { return BlockOpenHashIndexedObj2FloatMap.this.getKey(index); }
            public Index indexes() { return BlockOpenHashIndexedObj2FloatMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedObj2FloatMap.this.iterateIndexes(); }
    ...
    public IndexedFloatContainer values() {
        return new IndexedFloatContainer() {
            ...
            public boolean isEmpty() { return BlockOpenHashIndexedObj2FloatMap.this.isEmpty(); }
            public long head() { return BlockOpenHashIndexedObj2FloatMap.this.head(); }
            public float getDefaultValue() { return BlockOpenHashIndexedObj2FloatMap.this.getDefaultValue(); }
            public boolean has(long index) { return BlockOpenHashIndexedObj2FloatMap.this.has(index); }
            public Float get(long index) { return BlockOpenHashIndexedObj2FloatMap.this.getValue(index); }
            public float getFloat(long index) { return BlockOpenHashIndexedObj2FloatMap.this.getFloatValue(index); }
            public Index indexes() { return BlockOpenHashIndexedObj2FloatMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedObj2FloatMap.this.iterateIndexes(); }
     ...
     */
    @SuppressWarnings("unchecked")
    public BlockOpenHashIndexedObj2FloatMap<Key> clone() {
        BlockOpenHashIndexedObj2FloatMap<Key> c;
        try {
            c=(BlockOpenHashIndexedObj2FloatMap<Key>) super.clone();
        }

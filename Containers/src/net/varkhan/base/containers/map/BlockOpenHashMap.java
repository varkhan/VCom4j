...
public class BlockOpenHashMap<Key,Value> implements Map<Key,Value>, Serializable, Cloneable {
     ...
     */
    public BlockOpenHashMap(long size, int blockshift, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public BlockOpenHashMap(long size) {
    ...
    @SuppressWarnings("unchecked")
    public boolean add(final Key key, final Value val) {
        final long capa=this.capa;
        final long mixr=this.mixr;
        final long hash=this.strategy.hash(key);
        // Get a positive integer for the hash
    ...
    public Iterator<Map.Entry<Key,Value>> iterator() {
        return new Iterator<Map.Entry<Key,Value>>() {
            ...
            {
                Object k;
                if(c!=0) while(pos<BlockOpenHashMap.this.capa&&((k=_getKey(pos))==NULL||k==DEL)) pos++;
            }

            public boolean hasNext() {
                return c!=0&&pos<BlockOpenHashMap.this.capa;
            }
            ...
            public Map.Entry<Key,Value> next() {
                ...
                Object k;
                if(--c!=0) do pos++; while(pos<BlockOpenHashMap.this.capa&&((k=_getKey(pos))==NULL||k==DEL));
    ...
    public <Par> long visit(Visitor<Map.Entry<Key,Value>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            ...
            @SuppressWarnings("unchecked")
            long r=vis.invoke(new Entry((Key) k, (Value) _getVal(pos)), par);
            if(r<0) return c;
            c+=r;
            pos++;
        }
    ...
    public <Par> long visit(MapVisitor<Key,Value,Par> vis, Par par) {
        ...
        while(pos<capa) {
            ...
            @SuppressWarnings("unchecked")
            long r=vis.invoke((Key) k, (Value) _getVal(pos), par);
            if(r<0) return c;
            c+=r;
            pos++;
        }

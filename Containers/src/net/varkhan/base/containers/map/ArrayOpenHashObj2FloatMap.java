 ...
 * @time 9:43:13 PM
 */
public class ArrayOpenHashObj2FloatMap<Key> implements Obj2FloatMap<Key>, Serializable, Cloneable {
     ...
     */
    public ArrayOpenHashObj2FloatMap(long size, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public ArrayOpenHashObj2FloatMap(long size) {
     ...
     */
    @SuppressWarnings("unchecked")
    public ArrayOpenHashObj2FloatMap() {
...
//        for(int i=0;i<vals.length;i++) vals[i]=0;
    ...
    public boolean add(Map.Entry<Key,Float> item) {
        if(item==null) return false;
        return add(item.getKey(), item.getValue());
    }

    public boolean add(Obj2FloatMap.Entry<Key> item) {
        if(item==null) return false;
        return add(item.getKey(), item.getFloatValue());
    }

    public boolean add(Key key, Float val) {
        if(val==null) return false;
        return add(key,val.floatValue());
    }

    @SuppressWarnings("unchecked")
    public boolean add(final Key key, final float val) {
        final int capa=this.capa;
        final int mixr=this.mixr;
        final long hash=this.strategy.hash(key);
        // Get a positive integer for the hash
    ...
    @SuppressWarnings("unchecked")
    public Float get(final Key key) {
        final int capa=this.capa;
        final int mixr=this.mixr;
    ...
    @SuppressWarnings("unchecked")
    public float getFloat(final Key key) {
        final int capa=this.capa;
        final int mixr=this.mixr;
        final long hash=this.strategy.hash((Key) key);
        // Get a positive integer for the hash
...
//        vals[slot]=0;
     ...
     */
    private class Entry implements Obj2FloatMap.Entry<Key> {
        private final Key   key;
        private       float val;

        public Entry(Key key, float val) {
            this.key=key;
            this.val=val;
        }
    ...
    public Iterator<Obj2FloatMap.Entry<Key>> iterator() {
        return new Iterator<Obj2FloatMap.Entry<Key>>() {
            ...
            {
                Object k;
                if(c!=0) while(pos<ArrayOpenHashObj2FloatMap.this.capa&&((k=keys[pos])==NULL||k==DEL)) pos++;
            }

            public boolean hasNext() {
                return c!=0&&pos<ArrayOpenHashObj2FloatMap.this.capa;
            }

            @SuppressWarnings("unchecked")
            public Obj2FloatMap.Entry<Key> next() {
                if(!hasNext()) throw new NoSuchElementException();
                int index=last=pos;
                Key key=(Key) keys[index];
                float val=vals[pos];
                Object k;
                if(--c!=0) do pos++; while(pos<ArrayOpenHashObj2FloatMap.this.capa&&((k=keys[pos])==NULL||k==DEL));
...
//                vals[last]=0;
    ...
    public <Par> long visit(Visitor<Map.Entry<Key,Float>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            Object k=keys[pos];
...
//                vals[last]=0;
            ...
            public <Par> long visit(FloatVisitable.FloatVisitor<Par> vis, Par par) {
                long c=0;
                int pos=0;
                while(pos<capa) {
                    Object k=keys[pos];
     ...
     */
    @SuppressWarnings("unchecked")
    public ArrayOpenHashObj2FloatMap<Key> clone() {
        ArrayOpenHashObj2FloatMap<Key> c;
        try {
            c=(ArrayOpenHashObj2FloatMap<Key>) super.clone();
        }

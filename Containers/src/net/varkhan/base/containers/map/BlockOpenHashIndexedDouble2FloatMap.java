...
public class BlockOpenHashIndexedDouble2FloatMap implements IndexedDouble2FloatMap, Serializable, Cloneable {
     ...
     */
    public BlockOpenHashIndexedDouble2FloatMap(long size, int blockshift, float loadfact, float growfact, final DoubleHashingStrategy strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
     ...
     * @param size the expected number of elements in the map
     */
    public BlockOpenHashIndexedDouble2FloatMap(long size) {
     ...
     * Creates a new hash map.
     */
    public BlockOpenHashIndexedDouble2FloatMap() {
    ...
    private void _setEntry(long index, double k, float v) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        double[] kblock=this.keys[kblockpos];
        float[] vblock=this.vals[kblockpos];
    ...
    public long add(IndexedDouble2ObjMap.Entry<Float> item) {
        if(item==null) return -1;
        if(item.getValue()==null) return add(item.getDoubleKey(), defVal);
        else return add(item.getDoubleKey(), item.getValue().floatValue());
    }

    public long add(IndexedObj2FloatMap.Entry<Double> item) {
        if(item==null||item.getKey()==null) return -1;
        return add(item.getKey().doubleValue(), item.getFloatValue());
    }

    public long add(IndexedDouble2FloatMap.Entry item) {
        if(item==null) return -1;
        return add(item.getDoubleKey(), item.getFloatValue());
    }

    public long add(final Double key, final Float val) {
        if(key==null) return -1;
        if(val==null) return add(key.doubleValue(), defVal);
        return add(key.doubleValue(), val.floatValue());
    }
...
//            } while( sidx<0 || (sidx>0 && ! (hash == this.strategy.hash((Key)(kval=getKey(sidx-1))) && this.strategy.equal(key, (Key)kval))) );
    ...
    public Index indexes() {
        return new Index() {
            ...
            long c=BlockOpenHashIndexedDouble2FloatMap.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<BlockOpenHashIndexedDouble2FloatMap.this.capa&&_getIndex(next)<=0) next++;
                }
                return next<BlockOpenHashIndexedDouble2FloatMap.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==BlockOpenHashIndexedDouble2FloatMap.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<BlockOpenHashIndexedDouble2FloatMap.this.capa&&_getIndex(pos)<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<BlockOpenHashIndexedDouble2FloatMap.this.capa;
                    }
                    ...
                    public Long next() {
                        ...
                        if(--c!=0)
                            do pos++; while(pos<BlockOpenHashIndexedDouble2FloatMap.this.capa&&_getIndex(pos)<=0);
                        return idx;
                    }
    ...
    public Iterator<IndexedDouble2FloatMap.Entry> iterator() {
        return new Iterator<IndexedDouble2FloatMap.Entry>() {
            ...
            public IndexedDouble2FloatMap.Entry next() {
                if(!hasNext()) throw new NoSuchElementException();
                long idx=_getIndex(last=pos)-1;
                if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedDouble2FloatMap.this.capa&&_getIndex(pos)<=0);
    ...
    public IndexedDoubleContainer keys() {
        return new IndexedDoubleContainer() {
            public long size() { return BlockOpenHashIndexedDouble2FloatMap.this.size(); }
            public boolean isEmpty() { return BlockOpenHashIndexedDouble2FloatMap.this.isEmpty(); }
            public long head() { return BlockOpenHashIndexedDouble2FloatMap.this.head(); }
            public double getDefaultValue() { return BlockOpenHashIndexedDouble2FloatMap.this.getDefaultKey(); }
            public boolean has(long index) { return BlockOpenHashIndexedDouble2FloatMap.this.has(index); }
            public Double get(long index) { return BlockOpenHashIndexedDouble2FloatMap.this.getKey(index); }
            public double getDouble(long index) { return BlockOpenHashIndexedDouble2FloatMap.this.getDoubleKey(index); }
            public Index indexes() { return BlockOpenHashIndexedDouble2FloatMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedDouble2FloatMap.this.iterateIndexes(); }
    ...
    public IndexedFloatContainer values() {
        return new IndexedFloatContainer() {
            ...
            public boolean isEmpty() { return BlockOpenHashIndexedDouble2FloatMap.this.isEmpty(); }
            public long head() { return BlockOpenHashIndexedDouble2FloatMap.this.head(); }
            public float getDefaultValue() { return BlockOpenHashIndexedDouble2FloatMap.this.getDefaultValue(); }
            public boolean has(long index) { return BlockOpenHashIndexedDouble2FloatMap.this.has(index); }
            public Float get(long index) { return BlockOpenHashIndexedDouble2FloatMap.this.getValue(index); }
            public float getFloat(long index) { return BlockOpenHashIndexedDouble2FloatMap.this.getFloatValue(index); }
            public Index indexes() { return BlockOpenHashIndexedDouble2FloatMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedDouble2FloatMap.this.iterateIndexes(); }

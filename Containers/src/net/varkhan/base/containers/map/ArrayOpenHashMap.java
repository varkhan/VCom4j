...
public class ArrayOpenHashMap<Key,Value> implements Map<Key,Value>, Serializable {
     ...
     */
    public ArrayOpenHashMap(long size, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public Iterator<Map.Entry<Key,Value>> iterator() {
        return new Iterator<Map.Entry<Key,Value>>() {
            ...
            {
                Object k;
                if(c!=0) while(pos<ArrayOpenHashMap.this.capa&&((k=keys[pos])==NULL||k==DEL)) pos++;
            }

            public boolean hasNext() {
                return c!=0&&pos<ArrayOpenHashMap.this.capa;
            }
            ...
            public Map.Entry<Key,Value> next() {
                ...
                Object k;
                if(--c!=0) do pos++; while(pos<ArrayOpenHashMap.this.capa&&((k=keys[pos])==NULL||k==DEL));
    ...
    public <Par> long visit(Visitor<Map.Entry<Key,Value>,Par> vis, Par par) {
        ...
        while(pos<capa) {
            ...
            @SuppressWarnings("unchecked")
            long r=vis.invoke(new Entry((Key) k, (Value) vals[pos]), par);
            if(r<0) return c;
            c+=r;
            pos++;
        }

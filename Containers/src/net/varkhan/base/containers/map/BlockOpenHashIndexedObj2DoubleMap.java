...
public class BlockOpenHashIndexedObj2DoubleMap<Key> implements IndexedObj2DoubleMap<Key>, Serializable, Cloneable {
     ...
     */
    public BlockOpenHashIndexedObj2DoubleMap(long size, int blockshift, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    private double _getVal(long index) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        double[] vblock=this.vals[kblockpos];
        // No block ??
        return (vblock==null) ? defVal : vblock[kblockoff];
    }

    private void _setVal(long index, double v) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        double[] vblock=this.vals[kblockpos];
        if(vblock==null) this.vals[kblockpos]=vblock=new double[this.blocksize];
        vblock[kblockoff]=v;
    }
    ...
    public void setValue(long index, Double value) {
        ...
        else _setVal(index, value.doubleValue());
    }
...
//            } while( sidx<0 || (sidx>0 && ! (hash == this.strategy.hash((Key)(kval=getKey(sidx-1))) && this.strategy.equal(key, (Key)kval))) );
    ...
    protected void rehash(final long capa) {
        ...
        if(head<capa&&this.capa<capa) {
            ...
            if(bnum>this.keys.length) {
                ...
                this.keys=keys;
                final double[][] vals=new double[bnum][];
    ...
    private class Entry implements IndexedObj2DoubleMap.Entry<Key> {
        ...
        public Double getValue() { return _getVal(idx); }

        public double getDoubleValue() { return _getVal(idx); }

        public Double setValue(Double val) {
            double old=_getVal(idx);
            if(val==null) _setVal(idx, defVal);
            else _setVal(idx, val.doubleValue());
            return old;
        }
    ...
    public Index indexes() {
        return new Index() {
            ...
            long c=BlockOpenHashIndexedObj2DoubleMap.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<BlockOpenHashIndexedObj2DoubleMap.this.capa&&_getIndex(next)<=0) next++;
                }
                return next<BlockOpenHashIndexedObj2DoubleMap.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==BlockOpenHashIndexedObj2DoubleMap.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<BlockOpenHashIndexedObj2DoubleMap.this.capa&&_getIndex(pos)<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<BlockOpenHashIndexedObj2DoubleMap.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedObj2DoubleMap.this.capa&&_getIndex(pos)<=0);
                        return idx;
                    }
    ...
    public IndexedContainer<Key> keys() {
        return new IndexedContainer<Key>() {
            public long size() { return BlockOpenHashIndexedObj2DoubleMap.this.size(); }
            public boolean isEmpty() { return BlockOpenHashIndexedObj2DoubleMap.this.isEmpty(); }
            public long head() { return BlockOpenHashIndexedObj2DoubleMap.this.head(); }
            public boolean has(long index) { return BlockOpenHashIndexedObj2DoubleMap.this.has(index); }
            public Key get(long index) { return BlockOpenHashIndexedObj2DoubleMap.this.getKey(index); }
            public Index indexes() { return BlockOpenHashIndexedObj2DoubleMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedObj2DoubleMap.this.iterateIndexes(); }
    ...
    public IndexedDoubleContainer values() {
        return new IndexedDoubleContainer() {
            ...
            public boolean isEmpty() { return BlockOpenHashIndexedObj2DoubleMap.this.isEmpty(); }
            public long head() { return BlockOpenHashIndexedObj2DoubleMap.this.head(); }
            public double getDefaultValue() { return BlockOpenHashIndexedObj2DoubleMap.this.getDefaultValue(); }
            public boolean has(long index) { return BlockOpenHashIndexedObj2DoubleMap.this.has(index); }
            public Double get(long index) { return BlockOpenHashIndexedObj2DoubleMap.this.getValue(index); }
            public double getDouble(long index) { return BlockOpenHashIndexedObj2DoubleMap.this.getDoubleValue(index); }
            public Index indexes() { return BlockOpenHashIndexedObj2DoubleMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedObj2DoubleMap.this.iterateIndexes(); }

            public DoubleIterable.DoubleIterator iterator() {
                return new DoubleIterable.DoubleIterator() {
                    /** The position of the next entry to be returned. */
                    long pos=0;
                    /** The position of the last entry that has been returned. */
                    long last=-1;

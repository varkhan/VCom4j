...
public abstract class SyncIndexedContainer<Type> implements IndexedContainer<Type>, Serializable {
    ...
    /**
     * Iterates over all indexes in the container, using an {@link Index}.
     *
     ...
     * @return the sum of all positive return values from the visitor
     */
    public <Par> long visit(final IndexedVisitable.IndexedVisitor<Type,Par> vis, Par par) {
        rlock.lock();
        try {
            return container.visit(new IndexedVisitable.IndexedVisitor<Type,Par>() {
                private final long exid=opid;

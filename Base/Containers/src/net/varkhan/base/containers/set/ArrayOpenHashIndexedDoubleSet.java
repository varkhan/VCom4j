...
public class ArrayOpenHashIndexedDoubleSet implements IndexedDoubleSet, Serializable, Cloneable {
     ...
     */
    public ArrayOpenHashIndexedDoubleSet(long size, float loadfact, float growfact, final DoubleHashingStrategy strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
     ...
     * @param size the expected number of elements in the set
     */
    public ArrayOpenHashIndexedDoubleSet(long size) {
     ...
     * Creates a new hash set.
     */
    public ArrayOpenHashIndexedDoubleSet() {
...
//            } while( sidx<0 || (sidx>0 && ! (strategy.hash((Key)(kval=getKey(sidx-1)))==hash && strategy.equal((Key)key, (Key)kval) )) );
    ...
    public Index indexes() {
        return new Index() {
            ...
            int c=ArrayOpenHashIndexedDoubleSet.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<ArrayOpenHashIndexedDoubleSet.this.capa&&idxs[next]<=0) next++;
                }
                return next<ArrayOpenHashIndexedDoubleSet.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==ArrayOpenHashIndexedDoubleSet.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<ArrayOpenHashIndexedDoubleSet.this.capa&&idxs[pos]<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<ArrayOpenHashIndexedDoubleSet.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        int idx=idxs[(last=pos)]-1;
                        if(--c!=0) do pos++; while(pos<ArrayOpenHashIndexedDoubleSet.this.capa&&idxs[pos]<=0);
                        return (long) idx;
                    }
    ...
    public DoubleIterator iterator() {
        return new DoubleIterator() {
            ...
            public double nextValue() {
                if(!hasNext()) throw new NoSuchElementException();
                double key=keys[(idxs[(last=pos)]-1)];
                if(--c!=0) do pos++; while(pos<ArrayOpenHashIndexedDoubleSet.this.capa&&idxs[pos]<=0);
                return key;
            }
    ...
    /**
     * The base DoubleEntry object for this set
     */
            ...
            public Iterator<IndexedDoubleSet.DoubleEntry> iterator() {
                return new Iterator<IndexedDoubleSet.DoubleEntry>() {
                    ...
                    public IndexedDoubleSet.DoubleEntry next() {
                        ...
                        int idx=idxs[(last=pos)]-1;
                        if(--c!=0) do pos++; while(pos<ArrayOpenHashIndexedDoubleSet.this.capa&&idxs[pos]<=0);
                        return new DoubleEntry(idx, keys[idx]);
                    }

...
public class ArrayOpenHashIndexedSet<Key> implements IndexedSet<Key>, Serializable, Cloneable {
     ...
     */
    public ArrayOpenHashIndexedSet(long size, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public boolean has(long index) {
        ...
        return this.keys[(int) index]!=NULL;
    }
    ...
    public long index(final Key key) {
        ...
        if(sidx<0||(sidx>0&&!(this.strategy.hash((Key) (kval=this.keys[(sidx-1)]))==hash&&this.strategy.equal(key, (Key) kval)))) {
            ...
            while(sidx<0||(sidx>0&&!(this.strategy.hash((Key) (kval=keys[(sidx-1)]))==hash&&this.strategy.equal(key, (Key) kval))));
        }
    ...
    public void del(long index) {
        ...
        int pidx=(int) (+index+1);//(sidx>0)?sidx-1:(sidx<0)?-sidx-1:-1;
        int nidx=(int) (-index-1);
        // The key table value
//        Object kval;
        if(sidx!=0&&sidx!=pidx&&sidx!=nidx) {
//        if ( sidx<0 || (sidx>0 && ! (strategy.hash((Key)(kval=getKey(kidx=(sidx-1))))==hash && strategy.equal((Key)key, (Key)kval) )) ) {
            // The scan increment
            final int scan=(int) (ph%mixr)+1;
            do {
                slot+=scan;
...
//            } while( sidx<0 || (sidx>0 && ! (strategy.hash((Key)(kval=getKey(kidx=(sidx-1))))==hash && strategy.equal((Key)key, (Key)kval) )) );
    ...
    public Index indexes() {
        return new Index() {
            ...
            int c=ArrayOpenHashIndexedSet.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<ArrayOpenHashIndexedSet.this.capa&&idxs[next]<=0) next++;
                }
                return next<ArrayOpenHashIndexedSet.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==ArrayOpenHashIndexedSet.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<ArrayOpenHashIndexedSet.this.capa&&idxs[pos]<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<ArrayOpenHashIndexedSet.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        int idx=idxs[(last=pos)]-1;
                        if(--c!=0) do pos++; while(pos<ArrayOpenHashIndexedSet.this.capa&&idxs[pos]<=0);
                        return (long) idx;
                    }
            ...
            public Iterator<Key> iterator() {
                return new Iterator<Key>() {
                    ...
                    public Key next() {
                        ...
                        Object key=keys[(idxs[(last=(int) indexes[pos++])]-1)];
        ...
        return new Iterable<Key>() {
            public Iterator<Key> iterator() {
                return new Iterator<Key>() {
                    ...
                    @SuppressWarnings("unchecked")
                    public Key next() {
                        Object key=keys[(idxs[(last=iter.next().intValue())]-1)];
                        if(key==NULL) throw new NoSuchElementException();
                        return (Key) key;
                    }
        ...
        return new Iterable<Key>() {
            public Iterator<Key> iterator() {
                return new Iterator<Key>() {
                    ...
                    @SuppressWarnings("unchecked")
                    public Key next() {
                        Object key=keys[(idxs[(last=(int) iter.next())]-1)];
                        if(key==NULL) throw new NoSuchElementException();
                        return (Key) key;
                    }

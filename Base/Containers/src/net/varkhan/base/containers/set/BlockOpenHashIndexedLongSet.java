 ...
 * @time 9:43:13 PM
 */
public class BlockOpenHashIndexedLongSet implements IndexedLongSet, Serializable, Cloneable {
     ...
     */
    public BlockOpenHashIndexedLongSet(long size, int blockshift, float loadfact, float growfact, final LongHashingStrategy strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
     ...
     * @param size the expected number of elements in the set
     */
    public BlockOpenHashIndexedLongSet(long size) {
     ...
     * Creates a new hash set.
     */
    public BlockOpenHashIndexedLongSet() {
    ...
    public long getDefaultValue() { return defKey; }

    public void setDefaultValue(long def) { defKey=def; }
    ...
    private void _setKey(long index, long k) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        long[] kblock=this.keys[kblockpos];
...
//            } while( sidx<0 || (sidx>0 && ! (hash == this.strategy.hash((Key)(kval=getKey(sidx-1))) && this.strategy.equal(key, (Key)kval))) );
    ...
    public Index indexes() {
        return new Index() {
            ...
            long c=BlockOpenHashIndexedLongSet.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<BlockOpenHashIndexedLongSet.this.capa&&_getIndex(next)<=0) next++;
                }
                return next<BlockOpenHashIndexedLongSet.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==BlockOpenHashIndexedLongSet.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<BlockOpenHashIndexedLongSet.this.capa&&_getIndex(pos)<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<BlockOpenHashIndexedLongSet.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedLongSet.this.capa&&_getIndex(pos)<=0);
                        return idx;
                    }
    ...
    public LongIterator iterator() {
        return new LongIterator() {
            ...
            public long nextValue() {
                if(!hasNext()) throw new NoSuchElementException();
                long key=_getKey(_getIndex(last=pos)-1);
                if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedLongSet.this.capa&&_getIndex(pos)<=0);
                return key;
            }
     ...
     */
    private static class LongEntry implements IndexedLongSet.LongEntry {
        private final long idx;
        private final long key;

        public LongEntry(long idx, long key) {
            this.idx=idx;
            this.key=key;
        }
    ...
    public Iterable<IndexedLongSet.LongEntry> entries() {
        return new Iterable<IndexedLongSet.LongEntry>() {
            public Iterator<IndexedLongSet.LongEntry> iterator() {
                return new Iterator<IndexedLongSet.LongEntry>() {
                    /** The position of the next entry to be returned. */
                    long pos=0;
                    /** The position of the last entry that has been returned. */
                    long last=-1;
                    ...
                    public IndexedLongSet.LongEntry next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedLongSet.this.capa&&_getIndex(pos)<=0);
                        return new LongEntry(idx, _getKey(idx));
                    }
    ...
    public Iterable<IndexedLongSet.LongEntry> entries(final long[] indexes) {
        return new Iterable<IndexedLongSet.LongEntry>() {
            public Iterator<IndexedLongSet.LongEntry> iterator() {
                return new Iterator<IndexedLongSet.LongEntry>() {
                    /** The position of the next index */
    ...
    public Iterable<IndexedLongSet.LongEntry> entries(final Iterable<Long> indexes) {
        return new Iterable<IndexedLongSet.LongEntry>() {
            public Iterator<IndexedLongSet.LongEntry> iterator() {
                return new Iterator<IndexedLongSet.LongEntry>() {
                    /** An iterator over indexes */
    ...
    public Iterable<IndexedLongSet.LongEntry> entries(final Indexable indexes) {
        return new Iterable<IndexedLongSet.LongEntry>() {
            public Iterator<IndexedLongSet.LongEntry> iterator() {
                return new Iterator<IndexedLongSet.LongEntry>() {
                    /** An iterator over indexes */
     ...
     */
    public BlockOpenHashIndexedLongSet clone() {
        BlockOpenHashIndexedLongSet c;
        try {
            c=(BlockOpenHashIndexedLongSet) super.clone();
        }

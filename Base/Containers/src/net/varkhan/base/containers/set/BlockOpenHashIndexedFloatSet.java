 ...
 * @time 9:43:13 PM
 */
public class BlockOpenHashIndexedFloatSet implements IndexedFloatSet, Serializable, Cloneable {
     ...
     */
    public BlockOpenHashIndexedFloatSet(long size, int blockshift, float loadfact, float growfact, final FloatHashingStrategy strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
     ...
     * @param size the expected number of elements in the set
     */
    public BlockOpenHashIndexedFloatSet(long size) {
     ...
     * Creates a new hash set.
     */
    public BlockOpenHashIndexedFloatSet() {
    ...
    public float getDefaultValue() { return defKey; }

    public void setDefaultValue(float def) { defKey=def; }
    ...
    private void _setKey(long index, float k) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        float[] kblock=this.keys[kblockpos];
    ...
    public long add(final Float key) {
        if(key==null) return -1;
...
//            } while( sidx<0 || (sidx>0 && ! (hash == this.strategy.hash((Key)(kval=getKey(sidx-1))) && this.strategy.equal(key, (Key)kval))) );
    ...
    public Index indexes() {
        return new Index() {
            ...
            long c=BlockOpenHashIndexedFloatSet.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<BlockOpenHashIndexedFloatSet.this.capa&&_getIndex(next)<=0) next++;
                }
                return next<BlockOpenHashIndexedFloatSet.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==BlockOpenHashIndexedFloatSet.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<BlockOpenHashIndexedFloatSet.this.capa&&_getIndex(pos)<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<BlockOpenHashIndexedFloatSet.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedFloatSet.this.capa&&_getIndex(pos)<=0);
                        return idx;
                    }
    ...
    public FloatIterator iterator() {
        return new FloatIterator() {
            ...
            public float nextValue() {
                if(!hasNext()) throw new NoSuchElementException();
                float key=_getKey(_getIndex(last=pos)-1);
                if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedFloatSet.this.capa&&_getIndex(pos)<=0);
                return key;
            }
     ...
     */
    private static class FloatEntry implements IndexedFloatSet.FloatEntry {
        private final long  idx;
        private final float key;

        public FloatEntry(long idx, float key) {
            this.idx=idx;
            this.key=key;
        }
    ...
    public Iterable<IndexedFloatSet.FloatEntry> entries() {
        return new Iterable<IndexedFloatSet.FloatEntry>() {
            public Iterator<IndexedFloatSet.FloatEntry> iterator() {
                return new Iterator<IndexedFloatSet.FloatEntry>() {
                    /** The position of the next entry to be returned. */
                    long pos=0;
                    /** The position of the last entry that has been returned. */
                    long last=-1;
                    ...
                    public IndexedFloatSet.FloatEntry next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedFloatSet.this.capa&&_getIndex(pos)<=0);
                        return new FloatEntry(idx, _getKey(idx));
                    }
    ...
    public Iterable<IndexedFloatSet.FloatEntry> entries(final long[] indexes) {
        return new Iterable<IndexedFloatSet.FloatEntry>() {
            public Iterator<IndexedFloatSet.FloatEntry> iterator() {
                return new Iterator<IndexedFloatSet.FloatEntry>() {
                    /** The position of the next index */
    ...
    public Iterable<IndexedFloatSet.FloatEntry> entries(final Iterable<Long> indexes) {
        return new Iterable<IndexedFloatSet.FloatEntry>() {
            public Iterator<IndexedFloatSet.FloatEntry> iterator() {
                return new Iterator<IndexedFloatSet.FloatEntry>() {
                    /** An iterator over indexes */
    ...
    public Iterable<IndexedFloatSet.FloatEntry> entries(final Indexable indexes) {
        return new Iterable<IndexedFloatSet.FloatEntry>() {
            public Iterator<IndexedFloatSet.FloatEntry> iterator() {
                return new Iterator<IndexedFloatSet.FloatEntry>() {
                    /** An iterator over indexes */
     ...
     */
    public BlockOpenHashIndexedFloatSet clone() {
        BlockOpenHashIndexedFloatSet c;
        try {
            c=(BlockOpenHashIndexedFloatSet) super.clone();
        }
    ...
    public String toString() {
        ...
        while(pos<capa) {
            ...
            buf.append(' ').append(idx).append('@').append(k);
            pos++;
        }
        buf.append(' ').append('}');
        return buf.toString();

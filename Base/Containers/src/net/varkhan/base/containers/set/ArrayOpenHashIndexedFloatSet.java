...
public class ArrayOpenHashIndexedFloatSet implements IndexedFloatSet, Serializable, Cloneable {
     ...
     */
    public ArrayOpenHashIndexedFloatSet(long size, float loadfact, float growfact, final FloatHashingStrategy strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
     ...
     * @param size the expected number of elements in the set
     */
    public ArrayOpenHashIndexedFloatSet(long size) {
     ...
     * Creates a new hash set.
     */
    public ArrayOpenHashIndexedFloatSet() {
...
//            } while( sidx<0 || (sidx>0 && ! (strategy.hash((Key)(kval=getKey(sidx-1)))==hash && strategy.equal((Key)key, (Key)kval) )) );
    ...
    public Index indexes() {
        return new Index() {
            ...
            int c=ArrayOpenHashIndexedFloatSet.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<ArrayOpenHashIndexedFloatSet.this.capa&&idxs[next]<=0) next++;
                }
                return next<ArrayOpenHashIndexedFloatSet.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==ArrayOpenHashIndexedFloatSet.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<ArrayOpenHashIndexedFloatSet.this.capa&&idxs[pos]<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<ArrayOpenHashIndexedFloatSet.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        int idx=idxs[(last=pos)]-1;
                        if(--c!=0) do pos++; while(pos<ArrayOpenHashIndexedFloatSet.this.capa&&idxs[pos]<=0);
                        return (long) idx;
                    }
    ...
    public FloatIterator iterator() {
        return new FloatIterator() {
            ...
            public float nextValue() {
                if(!hasNext()) throw new NoSuchElementException();
                float key=keys[(idxs[(last=pos)]-1)];
                if(--c!=0) do pos++; while(pos<ArrayOpenHashIndexedFloatSet.this.capa&&idxs[pos]<=0);
                return key;
            }
    ...
    /**
     * The base FloatEntry object for this set
     */
            ...
            public Iterator<IndexedFloatSet.FloatEntry> iterator() {
                return new Iterator<IndexedFloatSet.FloatEntry>() {
                    ...
                    public IndexedFloatSet.FloatEntry next() {
                        ...
                        int idx=idxs[(last=pos)]-1;
                        if(--c!=0) do pos++; while(pos<ArrayOpenHashIndexedFloatSet.this.capa&&idxs[pos]<=0);
                        return new FloatEntry(idx, keys[idx]);
                    }

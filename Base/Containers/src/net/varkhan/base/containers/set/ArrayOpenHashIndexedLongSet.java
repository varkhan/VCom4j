...
public class ArrayOpenHashIndexedLongSet implements IndexedLongSet, Serializable, Cloneable {
     ...
     */
    public ArrayOpenHashIndexedLongSet(long size, float loadfact, float growfact, final LongHashingStrategy strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
     ...
     * @param size the expected number of elements in the set
     */
    public ArrayOpenHashIndexedLongSet(long size) {
     ...
     * Creates a new hash set.
     */
    public ArrayOpenHashIndexedLongSet() {
...
//            } while( sidx<0 || (sidx>0 && ! (strategy.hash((Key)(kval=getKey(sidx-1)))==hash && strategy.equal((Key)key, (Key)kval) )) );
    ...
    public Index indexes() {
        return new Index() {
            ...
            int c=ArrayOpenHashIndexedLongSet.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<ArrayOpenHashIndexedLongSet.this.capa&&idxs[next]<=0) next++;
                }
                return next<ArrayOpenHashIndexedLongSet.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==ArrayOpenHashIndexedLongSet.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<ArrayOpenHashIndexedLongSet.this.capa&&idxs[pos]<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<ArrayOpenHashIndexedLongSet.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        int idx=idxs[(last=pos)]-1;
                        if(--c!=0) do pos++; while(pos<ArrayOpenHashIndexedLongSet.this.capa&&idxs[pos]<=0);
                        return (long) idx;
                    }
    ...
    /**
     * The base LongEntry object for this set
     */
            ...
            public Iterator<IndexedLongSet.LongEntry> iterator() {
                return new Iterator<IndexedLongSet.LongEntry>() {
                    ...
                    public IndexedLongSet.LongEntry next() {
                        ...
                        int idx=idxs[(last=pos)]-1;
                        if(--c!=0) do pos++; while(pos<ArrayOpenHashIndexedLongSet.this.capa&&idxs[pos]<=0);
                        return new LongEntry(idx, keys[idx]);
                    }

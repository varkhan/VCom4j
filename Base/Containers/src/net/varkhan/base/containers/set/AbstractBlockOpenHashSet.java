 ...
 * @time 9:43:13 PM
 */
abstract class AbstractBlockOpenHashSet<Key> implements Set<Key>, Serializable, Cloneable {
    ...
    /**
     * The total number of entries in the table (always an entry in <code>{@link #PRIMES}</code>)
     */
    protected long capa;
     ...
     */
    public AbstractBlockOpenHashSet(long size, int blockshift, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public AbstractBlockOpenHashSet(long size) {
     ...
     */
    @SuppressWarnings("unchecked")
    public AbstractBlockOpenHashSet() {
    ...
    public Iterator<Key> iterator() {
        return new Iterator<Key>() {
            ...
            {
                Object k;
                if(c!=0) while(pos<AbstractBlockOpenHashSet.this.capa&&((k=_getKey(pos))==NULL||k==DEL)) pos++;
            }

            public boolean hasNext() {
                return c!=0&&pos<AbstractBlockOpenHashSet.this.capa;
            }
            ...
            public Key next() {
                ...
                Object k;
                if(--c!=0) do pos++; while(pos<AbstractBlockOpenHashSet.this.capa&&((k=_getKey(pos))==NULL||k==DEL));
                return key;
            }
    ...
    @SuppressWarnings("unchecked")
    public Object clone() {
        AbstractBlockOpenHashSet<Key> c;
        try {
            c=(AbstractBlockOpenHashSet<Key>) super.clone();
        }

...
abstract class AbstractBlockOpenHashIndexedSet<Key> implements IndexedSet<Key>, Serializable, Cloneable {
     ...
     */
    public AbstractBlockOpenHashIndexedSet(long size, int blockshift, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public AbstractBlockOpenHashIndexedSet(long size) {
     ...
     */
    @SuppressWarnings("unchecked")
    public AbstractBlockOpenHashIndexedSet() {
...
//            } while( sidx<0 || (sidx>0 && ! (hash == this.strategy.hash((Key)(kval=getKey(sidx-1))) && this.strategy.equal(key, (Key)kval))) );
    ...
    public Index indexes() {
        return new Index() {
            ...
            long c=AbstractBlockOpenHashIndexedSet.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<AbstractBlockOpenHashIndexedSet.this.capa&&_getIndex(next)<=0) next++;
                }
                return next<AbstractBlockOpenHashIndexedSet.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==AbstractBlockOpenHashIndexedSet.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<AbstractBlockOpenHashIndexedSet.this.capa&&_getIndex(pos)<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<AbstractBlockOpenHashIndexedSet.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<AbstractBlockOpenHashIndexedSet.this.capa&&_getIndex(pos)<=0);
                        return idx;
                    }
    ...
    @SuppressWarnings("unchecked")
    public Object clone() {
        AbstractBlockOpenHashIndexedSet<Key> c;
        try {
            c=(AbstractBlockOpenHashIndexedSet<Key>) super.clone();
        }

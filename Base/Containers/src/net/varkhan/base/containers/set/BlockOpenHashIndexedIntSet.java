...
public class BlockOpenHashIndexedIntSet implements IndexedIntSet, Serializable, Cloneable {
     ...
     */
    public BlockOpenHashIndexedIntSet(long size, int blockshift, float loadfact, float growfact, final IntHashingStrategy strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
     ...
     * @param size the expected number of elements in the set
     */
    public BlockOpenHashIndexedIntSet(long size) {
     ...
     * Creates a new hash set.
     */
    public BlockOpenHashIndexedIntSet() {
    ...
    public int getDefaultValue() { return defKey; }

    public void setDefaultValue(int def) { defKey=def; }
    ...
    private void _setKey(long index, int k) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        int[] kblock=this.keys[kblockpos];
    ...
    public long add(final Integer key) {
        if(key==null) return -1;
        return add(key.intValue());
    }

    public long add(final int key) {
        final long capa=this.capa;
        final long mixr=this.mixr;
        final long hash=this.strategy.hash(key);
        // Get a positive integer for the hash
...
//            } while( sidx<0 || (sidx>0 && ! (hash == this.strategy.hash((Key)(kval=getKey(sidx-1))) && this.strategy.equal(key, (Key)kval))) );
    ...
    public Index indexes() {
        return new Index() {
            ...
            long c=BlockOpenHashIndexedIntSet.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<BlockOpenHashIndexedIntSet.this.capa&&_getIndex(next)<=0) next++;
                }
                return next<BlockOpenHashIndexedIntSet.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==BlockOpenHashIndexedIntSet.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<BlockOpenHashIndexedIntSet.this.capa&&_getIndex(pos)<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<BlockOpenHashIndexedIntSet.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedIntSet.this.capa&&_getIndex(pos)<=0);
                        return idx;
                    }
    ...
    public IntIterator iterator() {
        return new IntIterator() {
            ...
            public int nextValue() {
                if(!hasNext()) throw new NoSuchElementException();
                int key=_getKey(_getIndex(last=pos)-1);
                if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedIntSet.this.capa&&_getIndex(pos)<=0);
                return key;
            }
     ...
     */
    private static class IntEntry implements IndexedIntSet.IntEntry {
        private final long idx;
        private final int  key;

        public IntEntry(long idx, int key) {
            this.idx=idx;
            this.key=key;
        }
    ...
    public Iterable<IndexedIntSet.IntEntry> entries() {
        return new Iterable<IndexedIntSet.IntEntry>() {
            public Iterator<IndexedIntSet.IntEntry> iterator() {
                return new Iterator<IndexedIntSet.IntEntry>() {
                    /** The position of the next entry to be returned. */
                    long pos=0;
                    /** The position of the last entry that has been returned. */
                    long last=-1;
                    ...
                    public IndexedIntSet.IntEntry next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedIntSet.this.capa&&_getIndex(pos)<=0);
                        return new IntEntry(idx, _getKey(idx));
                    }
    ...
    public Iterable<IndexedIntSet.IntEntry> entries(final long[] indexes) {
        return new Iterable<IndexedIntSet.IntEntry>() {
            public Iterator<IndexedIntSet.IntEntry> iterator() {
                return new Iterator<IndexedIntSet.IntEntry>() {
                    /** The position of the next index */
    ...
    public Iterable<IndexedIntSet.IntEntry> entries(final Iterable<Long> indexes) {
        return new Iterable<IndexedIntSet.IntEntry>() {
            public Iterator<IndexedIntSet.IntEntry> iterator() {
                return new Iterator<IndexedIntSet.IntEntry>() {
                    /** An iterator over indexes */
    ...
    public Iterable<IndexedIntSet.IntEntry> entries(final Indexable indexes) {
        return new Iterable<IndexedIntSet.IntEntry>() {
            public Iterator<IndexedIntSet.IntEntry> iterator() {
                return new Iterator<IndexedIntSet.IntEntry>() {
                    /** An iterator over indexes */

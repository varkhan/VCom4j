...
/**
 * <b>IndexedDoubleList using sparse segments.</b>.
 * <p/>
...
public class SparseIndexedDoubleList extends AbstractSparseIndexedList implements IndexedDoubleList, Externalizable, Cloneable {
     ...
     */
    public SparseIndexedDoubleList(int blockshift, double growthfact) {
     ...
     */
    public SparseIndexedDoubleList(int blockshift, double growthfact, IndexedList<? extends Number> list) {
     ...
     * @param list the IndexedList to copy
     */
    public SparseIndexedDoubleList(IndexedList<? extends Number> list) {
        this();
        Index it=list.indexes();
        while(it.hasNext()) {
            long id=it.next();
     ...
     * @param list the IndexedList to copy
     */
    public SparseIndexedDoubleList(IndexedDoubleList list) {
        this();
        Index it=list.indexes();
        while(it.hasNext()) {
            long id=it.next();
    ...
    /**
     * Builds a SparseIndexedDoubleList from an array of numbers.
     *
    ...
    /**
     * Builds a SparseIndexedDoubleList from an array of doubles.
     *
     * @param array the array to copy
     */
    ...
    public long set(long index, double val) {
        ...
        if(len>=block.length) {
            ...
            double[] newblock=new double[newlen];
    ...
    public <Par> long visit(Visitor<Double,Par> vis, Par par) {
        ...
        while(i<head) {
            ...
            int pos=getSparsePos(mask, (int) (i&blockmask));
            if(pos>0) {
                double obj=list[blockpos][pos-1];
     ...
     * followed by the set values (a variable number of {@code double}s, as defined by the mask)
     */
        ...
        if(head>0) {
            ...
            for(int i=0;i<headlen;i++) {
                ...
                if(mask==null) {
                    ...
                    for(int j=0;j<len;j++) out.writeDouble(block[j]);
                }
            }
        }
    }
        ...
        if(head>0) {
            ...
            for(int i=0;i<headlen;i++) {
                ...
                if(len==0) {
                    ...
                    final double[] block=list[i]=new double[len];
                    for(int j=0;j<len;j++) block[j]=in.readDouble();
                }
            }
        }
    }
    ...
    public boolean equals(Object obj) {
        if(!(obj instanceof SparseIndexedDoubleList)) return false;
        SparseIndexedDoubleList that=(SparseIndexedDoubleList) obj;
    ...
    public SparseIndexedDoubleList clone() {
        SparseIndexedDoubleList clone;
        try { clone=(SparseIndexedDoubleList) super.clone(); }

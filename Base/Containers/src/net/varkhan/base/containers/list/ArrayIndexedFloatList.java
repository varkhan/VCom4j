...
public class ArrayIndexedFloatList implements IndexedFloatList, Externalizable, Cloneable {
     ...
     */
    public ArrayIndexedFloatList(double gf) {
        if(gf<=1) gf=1.5;
        this.growthfact=gf;
        clear();
    }
     ...
     */
    public ArrayIndexedFloatList(double gf, IndexedFloatContainer list) {
        this(gf);
     ...
     * @param list the IndexedContainer to copy
     */
    public ArrayIndexedFloatList(IndexedFloatContainer list) {
        this();
        Index it=list.indexes();
        while(it.hasNext()) {
            long id=it.next();

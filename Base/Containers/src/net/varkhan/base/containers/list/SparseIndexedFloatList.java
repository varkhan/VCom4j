...
/**
 * <b>IndexedFloatList using sparse segments.</b>.
 * <p/>
...
public class SparseIndexedFloatList extends AbstractSparseIndexedList implements IndexedFloatList, Externalizable, Cloneable {
     ...
     */
    public SparseIndexedFloatList(int blockshift, double growthfact) {
     ...
     */
    public SparseIndexedFloatList(int blockshift, double growthfact, IndexedList<? extends Number> list) {
     ...
     * @param list the IndexedList to copy
     */
    public SparseIndexedFloatList(IndexedList<? extends Number> list) {
        this();
        Index it=list.indexes();
        while(it.hasNext()) {
            long id=it.next();
     ...
     * @param list the IndexedList to copy
     */
    public SparseIndexedFloatList(IndexedFloatList list) {
        this();
        Index it=list.indexes();
        while(it.hasNext()) {
            long id=it.next();
    ...
    /**
     * Builds a SparseIndexedFloatList from an array of numbers.
     *
    ...
    /**
     * Builds a SparseIndexedFloatList from an array of floats.
     *
     * @param array the array to copy
     */
    ...
    public long set(long index, float val) {
        ...
        if(len>=block.length) {
            ...
            float[] newblock=new float[newlen];
        ...
        if(head>0) {
            ...
            for(int i=0;i<headlen;i++) {
                ...
                if(mask==null) {
                    ...
                    for(int j=0;j<len;j++) out.writeFloat(block[j]);
                }
            }
        }
    }
        ...
        if(head>0) {
            ...
            for(int i=0;i<headlen;i++) {
                ...
                if(len==0) {
                    ...
                    final float[] block=list[i]=new float[len];
                    for(int j=0;j<len;j++) block[j]=in.readFloat();
                }
            }
        }
    }
        ...
        if(head>0) {
            ...
            for(int i=0;i<headlen;i++) {
                ...
                if(mask!=null) {
                    ...
                    final float[] block=list[i];
                    for(int j=0;j<len;j++) {
                        hash^=Float.floatToRawIntBits(block[j]);
                    }
                }
            }
        }
    ...
    public boolean equals(Object obj) {
        if(!(obj instanceof SparseIndexedFloatList)) return false;
        SparseIndexedFloatList that=(SparseIndexedFloatList) obj;
    ...
    public SparseIndexedFloatList clone() {
        SparseIndexedFloatList clone;
        try { clone=(SparseIndexedFloatList) super.clone(); }

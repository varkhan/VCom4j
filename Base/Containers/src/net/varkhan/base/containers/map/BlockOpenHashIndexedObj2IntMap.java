 ...
 * @time 9:43:13 PM
 */
public class BlockOpenHashIndexedObj2IntMap<Key> implements IndexedObj2IntMap<Key>, Serializable, Cloneable {
     ...
     */
    public BlockOpenHashIndexedObj2IntMap(long size, int blockshift, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public int getDefaultValue() { return defVal; }

    public void setDefaultValue(int def) { defVal=def; }
    ...
    private int _getVal(long index) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        int[] vblock=this.vals[kblockpos];
        // No block ??
    ...
    private void _setVal(long index, int v) {
        ...
        int kblockoff=(int) (index&this.blockmask);
        int[] vblock=this.vals[kblockpos];
        if(vblock==null) this.vals[kblockpos]=vblock=new int[this.blocksize];
        vblock[kblockoff]=v;
    }
    ...
    @SuppressWarnings("unchecked")
    public IndexedObj2IntMap.Entry<Key> get(long index) {
    ...
    public int getIntValue(long index) {
        if(index<0||index>=this.head) return defVal;
        if(!_hasKey(index)) return defVal;
        return _getVal(index);
    }

    public void setValue(long index, Integer value) {
        if(index<0||index>=this.head) return;
        if(!_hasKey(index)) return;
        if(value==null) _setVal(index, defVal);
        else _setVal(index, value.intValue());
    }

    public void setIntValue(long index, int value) {
        if(index<0||index>=this.head) return;
        if(!_hasKey(index)) return;
        _setVal(index, value);
    }

    public long add(IndexedMap.Entry<Key,Integer> item) {
        if(item==null) return -1;
        return add(item.getKey(), item.getValue());
    }

    public long add(IndexedObj2IntMap.Entry<Key> item) {
        if(item==null) return -1;
        return add(item.getKey(), item.getIntValue());
    }

    public long add(final Key key, final Integer val) {
        if(val==null) return add(key, defVal);
        return add(key, val.intValue());
    }

    @SuppressWarnings("unchecked")
    public long add(final Key key, final int val) {
        final long capa=this.capa;
        final long mixr=this.mixr;
        final long hash=this.strategy.hash(key);
        // Get a positive integer for the hash
...
//            } while( sidx<0 || (sidx>0 && ! (hash == this.strategy.hash((Key)(kval=getKey(sidx-1))) && this.strategy.equal(key, (Key)kval))) );
     ...
     */
    private class Entry implements IndexedObj2IntMap.Entry<Key> {
        ...
        public Integer getValue() { return _getVal(idx); }

        public int getIntValue() { return _getVal(idx); }

        public Integer setValue(Integer val) {
            int old=_getVal(idx);
            if(val==null) _setVal(idx, defVal);
            else _setVal(idx, val.intValue());
            return old;
        }
    ...
    public Index indexes() {
        return new Index() {
            ...
            long c=BlockOpenHashIndexedObj2IntMap.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<BlockOpenHashIndexedObj2IntMap.this.capa&&_getIndex(next)<=0) next++;
                }
                return next<BlockOpenHashIndexedObj2IntMap.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==BlockOpenHashIndexedObj2IntMap.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<BlockOpenHashIndexedObj2IntMap.this.capa&&_getIndex(pos)<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<BlockOpenHashIndexedObj2IntMap.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedObj2IntMap.this.capa&&_getIndex(pos)<=0);
                        return idx;
                    }
    ...
    public Iterator<IndexedObj2IntMap.Entry<Key>> iterator() {
        return new Iterator<IndexedObj2IntMap.Entry<Key>>() {
            /** The position of the next entry to be returned. */
            long pos=0;
            /** The position of the last entry that has been returned. */
            long last=-1;
            ...
            @SuppressWarnings("unchecked")
            public IndexedObj2IntMap.Entry<Key> next() {
                if(!hasNext()) throw new NoSuchElementException();
                long idx=_getIndex(last=pos)-1;
                if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedObj2IntMap.this.capa&&_getIndex(pos)<=0);
    ...
    public <Par> long visit(Visitor<IndexedMap.Entry<Key,Integer>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
    ...
    public <Par> long visit(IndexedVisitor<IndexedMap.Entry<Key,Integer>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            long idx=_getIndex(pos);
    ...
    public <Par> long visit(IndexedMapVisitor<Key,Integer,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            long idx=_getIndex(pos);
    ...
    public Iterable<IndexedObj2IntMap.Entry<Key>> iterate(final long[] indexes) {
        return new Iterable<IndexedObj2IntMap.Entry<Key>>() {
            public Iterator<IndexedObj2IntMap.Entry<Key>> iterator() {
                return new Iterator<IndexedObj2IntMap.Entry<Key>>() {
    ...
    public Iterable<IndexedObj2IntMap.Entry<Key>> iterate(final java.lang.Iterable<Long> indexes) {
        return new Iterable<IndexedObj2IntMap.Entry<Key>>() {
            public Iterator<IndexedObj2IntMap.Entry<Key>> iterator() {
                return new Iterator<IndexedObj2IntMap.Entry<Key>>() {
    ...
    public Iterable<IndexedObj2IntMap.Entry<Key>> iterate(final Indexable indexes) {
        return new Iterable<IndexedObj2IntMap.Entry<Key>>() {
            public Iterator<IndexedObj2IntMap.Entry<Key>> iterator() {
                return new Iterator<IndexedObj2IntMap.Entry<Key>>() {
                    /** An iterator over indexes */
    ...
    public IndexedContainer<Key> keys() {
        return new IndexedContainer<Key>() {
            public long size() { return BlockOpenHashIndexedObj2IntMap.this.size(); }
            public boolean isEmpty() { return BlockOpenHashIndexedObj2IntMap.this.isEmpty(); }
            public long head() { return BlockOpenHashIndexedObj2IntMap.this.head(); }
            public boolean has(long index) { return BlockOpenHashIndexedObj2IntMap.this.has(index); }
            public Key get(long index) { return BlockOpenHashIndexedObj2IntMap.this.getKey(index); }
            public Index indexes() { return BlockOpenHashIndexedObj2IntMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedObj2IntMap.this.iterateIndexes(); }
    ...
    public IndexedIntContainer values() {
        return new IndexedIntContainer() {
            public long size() { return BlockOpenHashIndexedObj2IntMap.this.size(); }
            public boolean isEmpty() { return BlockOpenHashIndexedObj2IntMap.this.isEmpty(); }
            public long head() { return BlockOpenHashIndexedObj2IntMap.this.head(); }
            public int getDefaultValue() { return BlockOpenHashIndexedObj2IntMap.this.getDefaultValue(); }
            public boolean has(long index) { return BlockOpenHashIndexedObj2IntMap.this.has(index); }
            public Integer get(long index) { return BlockOpenHashIndexedObj2IntMap.this.getValue(index); }
            public int getInt(long index) { return BlockOpenHashIndexedObj2IntMap.this.getIntValue(index); }
            public Index indexes() { return BlockOpenHashIndexedObj2IntMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedObj2IntMap.this.iterateIndexes(); }
     ...
     */
    @SuppressWarnings("unchecked")
    public BlockOpenHashIndexedObj2IntMap<Key> clone() {
        BlockOpenHashIndexedObj2IntMap<Key> c;
        try {
            c=(BlockOpenHashIndexedObj2IntMap<Key>) super.clone();
        }

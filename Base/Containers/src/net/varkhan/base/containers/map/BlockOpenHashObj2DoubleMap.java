...
public class BlockOpenHashObj2DoubleMap<Key> implements Obj2DoubleMap<Key>, Serializable, Cloneable {
     ...
     */
    public BlockOpenHashObj2DoubleMap(long size, int blockshift, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public BlockOpenHashObj2DoubleMap(long size) {
     ...
     */
    @SuppressWarnings("unchecked")
    public BlockOpenHashObj2DoubleMap() {
    ...
    private void _setEntry(long index, Object k, double v) {
        ...
        if(vblock==null) {
            vals[kblockpos]=vblock=new double[blocksize];
        }
        kblock[kblockoff]=k;
        vblock[kblockoff]=v;
    }
    ...
    public boolean add(Map.Entry<Key,Double> item) {
        if(item==null) return false;
        return add(item.getKey(), item.getValue());
    }

    public boolean add(Obj2DoubleMap.Entry<Key> item) {
        if(item==null) return false;
        return add(item.getKey(), item.getDoubleValue());
    }

    public boolean add(Key key, Double val) {
        if(val==null) return false;
        return add(key,val.doubleValue());
    }

    @SuppressWarnings("unchecked")
    public boolean add(final Key key, final double val) {
        final long capa=this.capa;
        final long mixr=this.mixr;
        final long hash=this.strategy.hash(key);
        // Get a positive integer for the hash
    ...
    @SuppressWarnings("unchecked")
    public Double get(final Key key) {
        final long capa=this.capa;
        final long mixr=this.mixr;
    ...
    protected void rehash(final long capa) {
        ...
        final double[][] vals=new double[(int) (capa>>>blockshift)+1][];
        while(s-->0) {
            Object key;
     ...
     */
    private class Entry implements Obj2DoubleMap.Entry<Key> {
        private final Key    key;
        private       double val;

        public Entry(Key key, double val) {
            this.key=key;
            this.val=val;
        }
    ...
    public Iterator<Obj2DoubleMap.Entry<Key>> iterator() {
        return new Iterator<Obj2DoubleMap.Entry<Key>>() {
            ...
            {
                Object k;
                if(c!=0) while(pos<BlockOpenHashObj2DoubleMap.this.capa&&((k=_getKey(pos))==NULL||k==DEL)) pos++;
            }

            public boolean hasNext() {
                return c!=0&&pos<BlockOpenHashObj2DoubleMap.this.capa;
            }

            @SuppressWarnings("unchecked")
            public Obj2DoubleMap.Entry<Key> next() {
                if(!hasNext()) throw new NoSuchElementException();
                Key key=(Key) _getKey(last=pos);
                double val=_getVal(pos);
                Object k;
                if(--c!=0) do pos++; while(pos<BlockOpenHashObj2DoubleMap.this.capa&&((k=_getKey(pos))==NULL||k==DEL));
    ...
    public <Par> long visit(Visitor<Map.Entry<Key,Double>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
    ...
    public <Par> long visit(MapVisitor<Key,Double,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            Object k=_getKey(pos);
    ...
    public DoubleContainer values() {
        return new DoubleContainer() {
            ...
            public <Par> long visit(DoubleVisitable.DoubleVisitor<Par> vis, Par par) {
                long c=0;
                int pos=0;
                while(pos<capa) {
                    Object k=_getKey(pos);

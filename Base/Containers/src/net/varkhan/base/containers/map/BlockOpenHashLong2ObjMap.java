 ...
 * @time 9:43:13 PM
 */
public class BlockOpenHashLong2ObjMap<Value> implements Long2ObjMap<Value>, Serializable, Cloneable {
     ...
     */
    public BlockOpenHashLong2ObjMap(long size, int blockshift, float loadfact, float growfact, final LongHashingStrategy strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public BlockOpenHashLong2ObjMap(long size) {
     ...
     */
    @SuppressWarnings("unchecked")
    public BlockOpenHashLong2ObjMap() {
    ...
    private void _setEntry(long index, long k, Object v) {
        ...
        Object[] vblock=vals[kblockpos];
        if(vblock==null) {
            vals[kblockpos]=vblock=new Object[blocksize];
        }
        kblock[kblockoff]=k;
        vblock[kblockoff]=v;
    }
    ...
    protected void rehash(final long capa) {
        ...
        final Object[][] vals=new Object[(int) (capa>>>blockshift)+1][];
        while(s-->0) {
            long key;
    ...
    private class Entry implements Long2ObjMap.Entry<Value> {
        ...
        public Entry(final long key, Value val) {
            this.key=key;
            this.val=val;
        }
    ...
    public Iterator<Long2ObjMap.Entry<Value>> iterator() {
        return new Iterator<Long2ObjMap.Entry<Value>>() {
            ...
            {
                long k;
                if(c!=0) while(pos<BlockOpenHashLong2ObjMap.this.capa&&((k=_getKey(pos))==NULL||k==DEL)) pos++;
            }

            public boolean hasNext() {
                return c!=0&&pos<BlockOpenHashLong2ObjMap.this.capa;
            }
            ...
            public Long2ObjMap.Entry<Value> next() {
                ...
                Value val=(Value) _getVal(pos);
                long k;
                if(--c!=0) do pos++; while(pos<BlockOpenHashLong2ObjMap.this.capa&&((k=_getKey(pos))==NULL||k==DEL));
    ...
    public <Par> long visit(Visitor<Map.Entry<Long,Value>,Par> vis, Par par) {
        ...
        while(pos<capa) {
            ...
            @SuppressWarnings("unchecked")
            long r=vis.invoke(new Entry(k, (Value) _getVal(pos)), par);
            if(r<0) return c;
            c+=r;
            pos++;
        }
    ...
    public <Par> long visit(MapVisitor<Long,Value,Par> vis, Par par) {
        ...
        while(pos<capa) {
            ...
            @SuppressWarnings("unchecked")
            long r=vis.invoke(k, (Value) _getVal(pos), par);
            if(r<0) return c;
            c+=r;
            pos++;
        }
        ...
        return new Container<Value>() {
            ...
            public <Par> long visit(Visitor<Value,Par> vis, Par par) {
                ...
                while(pos<capa) {
                    ...
                    long r=vis.invoke((Value) _getVal(pos), par);
                    if(r<0) return c;
                    c+=r;
                    pos++;
                }
     ...
     */
    public BlockOpenHashLong2ObjMap clone() {
        BlockOpenHashLong2ObjMap c;
        try {
            c=(BlockOpenHashLong2ObjMap) super.clone();
        }

 ...
 * @time 9:43:13 PM
 */
public class ArrayOpenHashObj2LongMap<Key> implements Obj2LongMap<Key>, Serializable, Cloneable {
     ...
     */
    public ArrayOpenHashObj2LongMap(long size, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    public ArrayOpenHashObj2LongMap(long size) {
     ...
     */
    @SuppressWarnings("unchecked")
    public ArrayOpenHashObj2LongMap() {
...
//        for(int i=0;i<vals.length;i++) vals[i]=0;
    ...
    public boolean add(Map.Entry<Key,Long> item) {
        if(item==null) return false;
        return add(item.getKey(), item.getValue());
    }

    public boolean add(Obj2LongMap.Entry<Key> item) {
        if(item==null) return false;
        return add(item.getKey(), item.getLongValue());
    }

    public boolean add(Key key, Long val) {
        if(val==null) return false;
        return add(key,val.longValue());
    }

    @SuppressWarnings("unchecked")
    public boolean add(final Key key, final long val) {
        final int capa=this.capa;
        final int mixr=this.mixr;
        final long hash=this.strategy.hash(key);
        // Get a positive integer for the hash
...
//        vals[slot]=0;
     ...
     */
    private class Entry implements Obj2LongMap.Entry<Key> {
        private final Key   key;
        private       long val;

        public Entry(Key key, long val) {
            this.key=key;
            this.val=val;
        }
    ...
    public Iterator<Obj2LongMap.Entry<Key>> iterator() {
        return new Iterator<Obj2LongMap.Entry<Key>>() {
            ...
            {
                Object k;
                if(c!=0) while(pos<ArrayOpenHashObj2LongMap.this.capa&&((k=keys[pos])==NULL||k==DEL)) pos++;
            }

            public boolean hasNext() {
                return c!=0&&pos<ArrayOpenHashObj2LongMap.this.capa;
            }

            @SuppressWarnings("unchecked")
            public Obj2LongMap.Entry<Key> next() {
                if(!hasNext()) throw new NoSuchElementException();
                int index=last=pos;
                Key key=(Key) keys[index];
                long val=vals[pos];
                Object k;
                if(--c!=0) do pos++; while(pos<ArrayOpenHashObj2LongMap.this.capa&&((k=keys[pos])==NULL||k==DEL));
...
//                vals[last]=0;
    ...
    public <Par> long visit(Visitor<Map.Entry<Key,Long>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            Object k=keys[pos];
    ...
    public <Par> long visit(MapVisitor<Key,Long,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            Object k=keys[pos];
...
//                vals[last]=0;
            ...
            public <Par> long visit(LongVisitable.LongVisitor<Par> vis, Par par) {
                long c=0;
                int pos=0;
                while(pos<capa) {
                    Object k=keys[pos];
     ...
     */
    @SuppressWarnings("unchecked")
    public ArrayOpenHashObj2LongMap<Key> clone() {
        ArrayOpenHashObj2LongMap<Key> c;
        try {
            c=(ArrayOpenHashObj2LongMap<Key>) super.clone();
        }

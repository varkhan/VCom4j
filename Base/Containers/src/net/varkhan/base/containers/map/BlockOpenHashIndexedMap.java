 ...
 * @time 9:43:13 PM
 */
public class BlockOpenHashIndexedMap<Key,Value> implements IndexedMap<Key,Value>, Serializable, Cloneable {
     ...
     */
    public BlockOpenHashIndexedMap(long size, int blockshift, float loadfact, float growfact, final HashingStrategy<Key> strategy) {
...
//        if(size<0) throw new IllegalArgumentException("Hash table size must be nonnegative");
    ...
    private void _setEntry(long index, Object k, Object v) {
        int kblockpos=(int) (index>>>this.blockshift);
        int kblockoff=(int) (index&this.blockmask);
        Object[] kblock=this.keys[kblockpos];
        Object[] vblock=this.vals[kblockpos];
    ...
    @SuppressWarnings("unchecked")
    public long add(final Key key, final Value val) {
        final long capa=this.capa;
        final long mixr=this.mixr;
        final long hash=this.strategy.hash(key);
        // Get a positive integer for the hash
...
//            } while( sidx<0 || (sidx>0 && ! (hash == this.strategy.hash((Key)(kval=getKey(sidx-1))) && this.strategy.equal(key, (Key)kval))) );
     ...
     */
    @SuppressWarnings("unchecked")
    private class Entry implements IndexedMap.Entry<Key,Value> {
    ...
    public Index indexes() {
        return new Index() {
            ...
            long c=BlockOpenHashIndexedMap.this.size;
            ...
            public boolean hasNext() {
                ...
                if(next==-1) {
                    next=curr+1;
                    while(next<BlockOpenHashIndexedMap.this.capa&&_getIndex(next)<=0) next++;
                }
                return next<BlockOpenHashIndexedMap.this.capa;
            }
            ...
            public boolean hasPrevious() {
                if(c==BlockOpenHashIndexedMap.this.size) return false;
                if(last==-1) {
                    last=curr-1;
        ...
        return new java.lang.Iterable<Long>() {
            public java.util.Iterator<Long> iterator() {
                return new java.util.Iterator<Long>() {
                    ...
                    {
                        if(c!=0) while(pos<BlockOpenHashIndexedMap.this.capa&&_getIndex(pos)<=0) pos++;
                    }

                    public boolean hasNext() {
                        return c!=0&&pos<BlockOpenHashIndexedMap.this.capa;
                    }

                    public Long next() {
                        if(!hasNext()) throw new NoSuchElementException();
                        long idx=_getIndex(last=pos)-1;
                        if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedMap.this.capa&&_getIndex(pos)<=0);
                        return idx;
                    }
    ...
    public Iterator<IndexedMap.Entry<Key,Value>> iterator() {
        return new Iterator<IndexedMap.Entry<Key,Value>>() {
            /** The position of the next entry to be returned. */
            long pos=0;
            /** The position of the last entry that has been returned. */
            long last=-1;
            ...
            @SuppressWarnings("unchecked")
            public IndexedMap.Entry<Key,Value> next() {
                if(!hasNext()) throw new NoSuchElementException();
                long idx=_getIndex(last=pos)-1;
                if(--c!=0) do pos++; while(pos<BlockOpenHashIndexedMap.this.capa&&_getIndex(pos)<=0);
    ...
    public <Par> long visit(Visitor<IndexedMap.Entry<Key,Value>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
    ...
    public <Par> long visit(IndexedVisitor<IndexedMap.Entry<Key,Value>,Par> vis, Par par) {
        long c=0;
        int pos=0;
        while(pos<capa) {
            long idx=_getIndex(pos);
    ...
    public Iterable<IndexedMap.Entry<Key,Value>> iterate(final java.lang.Iterable<Long> indexes) {
        return new Iterable<IndexedMap.Entry<Key,Value>>() {
            public Iterator<IndexedMap.Entry<Key,Value>> iterator() {
                return new Iterator<IndexedMap.Entry<Key,Value>>() {
    ...
    public Iterable<IndexedMap.Entry<Key,Value>> iterate(final Indexable indexes) {
        return new Iterable<IndexedMap.Entry<Key,Value>>() {
            public Iterator<IndexedMap.Entry<Key,Value>> iterator() {
                return new Iterator<IndexedMap.Entry<Key,Value>>() {
                    /** An iterator over indexes */
    ...
    public IndexedContainer<Key> keys() {
        return new IndexedContainer<Key>() {
            public long size() { return BlockOpenHashIndexedMap.this.size(); }
            public boolean isEmpty() { return BlockOpenHashIndexedMap.this.isEmpty(); }
            public long head() { return BlockOpenHashIndexedMap.this.head(); }
            public boolean has(long index) { return BlockOpenHashIndexedMap.this.has(index); }
            public Key get(long index) { return BlockOpenHashIndexedMap.this.getKey(index); }
            public Index indexes() { return BlockOpenHashIndexedMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedMap.this.iterateIndexes(); }
    ...
    public IndexedContainer<Value> values() {
        return new IndexedContainer<Value>() {
            ...
            public long head() { return BlockOpenHashIndexedMap.this.head(); }
            public boolean has(long index) { return BlockOpenHashIndexedMap.this.has(index); }
            public Value get(long index) { return BlockOpenHashIndexedMap.this.getValue(index); }
            public Index indexes() { return BlockOpenHashIndexedMap.this.indexes(); }
            public java.lang.Iterable<Long> iterateIndexes() { return BlockOpenHashIndexedMap.this.iterateIndexes(); }
     ...
     */
    @SuppressWarnings("unchecked")
    public BlockOpenHashIndexedMap<Key,Value> clone() {
        BlockOpenHashIndexedMap<Key,Value> c;
        try {
            c=(BlockOpenHashIndexedMap<Key,Value>) super.clone();
        }

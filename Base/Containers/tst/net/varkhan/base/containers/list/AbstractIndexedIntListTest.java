...
public abstract class AbstractIndexedIntListTest extends TestCase {
    ...
    public void featureTestAddO(Random rand, int[] vals, IndexedIntList ilst, int defVal) throws Exception {
        Set<Integer> add=new HashSet<Integer>();
        long size=0;
        long head=0;
        long[] pos = new long[vals.length];
        while(add.size()<vals.length) {
            ...
            size++;
            long idx=ilst.add(Integer.valueOf(vals[i]));
            if(idx>=head) head=idx+1;
            assertTrue("add("+i+")", idx>=0);
            assertEquals("size()", size, ilst.size());
            assertEquals("head()", head, ilst.head());
        ...
        assertEquals("head()", head, ilst.head());
        for(int i=0; i<pos.length; i++) {
            if(pos[i]>=0) assertEquals("get(i)",Integer.valueOf(vals[i]),ilst.get(pos[i]));
        }
        System.out.println("add(T) OK");
    }

    public <T> void featureTestSetO(Random rand, int[] vals, IndexedIntList ilst, int defVal) throws Exception {
        Set<Integer> add=new HashSet<Integer>();
        long size=0;
        long head=0;
        while(add.size()<vals.length) {
            ...
            size++;
            if(i>=head) head=i+1;
            assertEquals("set("+i+")", i, ilst.set(i, Integer.valueOf(vals[i])));
        }
        assertEquals("size()", ilst.size(), size);
        assertEquals("head()", ilst.head(), head);
        Map<Integer,Integer> ref = new HashMap<Integer,Integer>();
        ...
        for(Map.Entry<Integer,Integer> e: ref.entrySet()) {
            int j= e.getKey();
            int i= e.getValue();
            if(vals[i]!=defVal) assertEquals("get("+j+")", Integer.valueOf(vals[i]), ilst.get(j));
        }
        System.out.println("set(long,T) OK");
    }
    ...
    public void featureTestVisit(Random rand, int[] vals, IndexedIntList ilst, int defVal) throws Exception {
        ...
        Visitable.Visitor<Integer,java.util.Set<Integer>> v= new Visitable.Visitor<Integer,java.util.Set<Integer>>() {
            @Override
            public long invoke(Integer obj, java.util.Set<Integer> ts) {
                return ts.contains(obj)?1:-1;
            }
        };
    ...
    public void featureTestVisitIndexed(Random rand, int[] vals, IndexedIntList ilst, int defVal) throws Exception {
        ...
        assertEquals("size()==visit()",ilst.size(),ilst.visit(vv, ref));
        IndexedVisitable.IndexedVisitor<Integer,java.util.Map<Long,Integer>> v= new IndexedVisitable.IndexedVisitor<Integer,java.util.Map<Long,Integer>>() {
            @Override
            public long invoke(long idx, Integer obj, java.util.Map<Long,Integer> ts) {
